    ...
    <table width="100%">
        <tr>
            <td style="text-align: right;">
                <small>
                    <strong>
                        Generated AT: {{ isset($stockPurchasePayment) ? dateTimeFormat($stockPurchasePayment->created_at) : dateTimeFormat(now()) }}</strong>
                </small>
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td style="color: #d1652b; width: 33.33%">
                <h4 style="margin: 0; padding: 0;">
                    ...
                    <span style="color: black; border-bottom: 2px dotted #d1652b">{{ $stockPurchasePayment->voucher_number }}</span>
                </h4>
            </td>
            ...
            <td style="color: #d1652b; width: 33.33%; text-align:right;">
                <h4>
                    <span class="voucher-text">Date</span> <span style="color: black; border-bottom: 2px dotted #d1652b; direction: ltr;">{{ dateFormat($stockPurchasePayment->payment_date) }}</span>
                </h4>
            </td>
        </tr>
    </table>
...
<div>
    <div class="">
        <h4 style="color: #d1652b; min-width: 70px; border-bottom: 2px dotted #d1652b; padding-bottom: 5px; margin: 5px 0;">
            Supplier Name:
            <span style="color: black; margin-left: 20px; font-weight: bold;">{{ data_get($stockPurchasePayment->is_bulk_payment ? $paymentData->first() : $stockPurchasePayment, 'stockPurchase.supplier.name')  }}</span>
        </h4>
    </div>
    <div class="">
        <h4 style="color: #d1652b; min-width: 70px; border-bottom: 2px dotted #d1652b; padding-bottom: 5px; margin: 5px 0;">
            Paid To:
            <span style="color: black; margin-left: 20px; font-weight: bold;">{{ data_get($stockPurchasePayment, 'paid_to')  }}</span>
        </h4>
    </div>
    <div class="">
...
<div>
    <table cellspacing="0" cellpadding="1" style="width: 100%; font-size: 12px; margin-top: 20px; border-collapse: collapse;">
        <thead>
        <tr>
        ...
        <tbody>
        @php $totalsByCurrency = [];  @endphp
        @foreach($paymentData as $key => $value)
            @php
                $purchaseCurrency = $value->stockPurchase->currency ?? 'AED';
                $totalsByCurrency[$purchaseCurrency] = ($totalsByCurrency[$purchaseCurrency] ?? 0) + $value->amount;
            @endphp
            <tr>
                <td colspan="8">
            ...
            <tr>
                ...
                <td style="text-align: center;">{{ $value->stockPurchase->batch_number }}</td>
                <td style="text-align: center;">{{ priceFormat($value->stockPurchase->total_amount, 2, '') }}</td>
                <td style="text-align: center;">{{ priceFormat($value->stockPurchase->payments->where('id', '<', $value->id)->sum('amount'), 2, '') }}</td>
                <td style="text-align: center;">{{ priceFormat($value->amount, 2, '') }}</td>
                <td style="text-align: center;">{{ priceFormat($value->stockPurchase->total_due, 2, '') }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
...
<table style="width: 100%;">
    ...
    <tr>
        <td style="padding-left: 200px; padding-top: 15px;">
            <div>
                <img style="z-index: -100; width: 150px; margin-top: -20px;" src="{{ config('setting.basic_information.paid_watter_mark') }}" alt="Received">
            </div>
        </td>
        ...
        <td style=" padding-left: 20px; padding-top: -30px; padding-right: 0;">
            <div style="">
                <table class="add-border" style="padding-left: 100px; width: 350px;">
                    ...
                    <tr>
                        @if($loop->first)
                        <td rowspan="{{ count($totalsByCurrency) }}" style="text-align: center; padding: 10px;">
                            TOTAL PAID
                        </td>
                        @endif
                        <td style="text-align: right; padding-right: 5px;">
                            {{ $cur }}: {{ priceFormat($total, 2, '') }}
                        </td>
                    </tr>
                    @endforeach
                </table>
...
<h4 style="color: #d1652b; border-bottom: 2px dotted #d1652b;">
    Note : @if(!empty($stockPurchasePayment->notes))
        <span style="color: black; ">{{ $stockPurchasePayment->notes }} </span>
    @endif
</h4>
<br>
<br>
