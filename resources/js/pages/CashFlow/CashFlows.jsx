...
import { cashFlowFilterType } from '@/lib/options';
...
import CashFlowActionButton from './CashFlowActionButton';
import { CashFlowMobileDetailSheet } from './CashFlowMobileDetailSheet';
import { CashFlowMobileViewCard } from './CashFlowMobileViewCard';
import CashFlowModal from './CashFlowModal';

const breadcrumbs = [{ label: 'Cash Flow' }];
...
export default function CashFlows(props) {
    ...
    const portal = usePortal('cash-flow-modal');
                    ...
                    return (
                        <div className="space-y-3 p-1">
                            <SmartDatePicker
                                ...
                                value={values?.date || ''}
                                onChange={(value) =>
                                    onChange('date', formatDateToYMD(value))
                                }
                                includeTime={false}
                    ...
                    return (
                        <div className="space-y-3 p-1">
                            <IconInput
                                ...
                                onChange={(e) =>
                                    onChange('voucher_number', e.target.value)
                                }
    ...
    const handleSearch = (searchData) => {
        ...
        router.visit('/cashflow-transactions', {
            method: 'get',
    ...
    const handleDelete = async (data) => {
        const ok = await confirm({
            title: 'Delete this cashflow transaction?',
            description: `The cashflow transaction <span class="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-bold">"${data.name}"</span> will be permanently removed. This action cannot be undone.`,
            variant: 'destructive',
            confirmText: 'Yes, Delete It',
        });

        if (ok) {
            router.delete(`/cashflow-transactions/${data.id}`, {
                onSuccess: (response) => {
