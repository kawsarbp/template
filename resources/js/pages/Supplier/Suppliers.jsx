...
import { SupplierMobileDetailSheet } from './SupplierMobileDetailSheet';
import { SupplierMobileViewCard } from './SupplierMobileViewCard';
...
export default function Suppliers(props) {
    ...
    const handleDelete = async (data) => {
        const ok = await confirm({
            ...
            description: `The supplier <span class="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-bold">"${data.name}"</span> will be permanently removed. This action cannot be undone.`,
            variant: 'destructive',
            confirmText: 'Yes, Delete It',
        });

        if (ok) {
            router.delete(`/suppliers/${data.id}`, {
                onSuccess: (response) => {
            ...
            <div className="relative bg-background font-sans text-foreground">
                <div className="mb-4 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    ...
                    <div className="flex flex-wrap items-end justify-end gap-2">
                        ...
                        {permissions['export excel supplier'] && (
                            <Button
                                onClick={() =>
                                    fileRouteHandler({
