...
import { logEventOptions, logTypeOptions } from '@/lib/options';
...
import { AuditLogMobileViewCard } from './AuditLogMobileViewCard';
...
export default function AuditLogs(props) {
    ...
    const tableColumnsDefinitions = useMemo(
        () => [
...

            {
                ...
                cell: ({ row }) => (
                    <div
                        className="font-medium text-muted-foreground [&_a]:font-semibold [&_a]:text-primary [&_a]:hover:underline"
                ...
                cellMobileView: (mobileViewData) => (
                    <DetailRow label="Description">
                        <div
                            className="font-medium text-muted-foreground [&_a]:font-semibold [&_a]:text-primary [&_a]:hover:underline"
                            dangerouslySetInnerHTML={{
                                __html: mobileViewData.description,
                            }}
                        />
                    </DetailRow>
                ),
                    ...
                    return (
                        <div className="space-y-3 p-1">
                            <BasicSelect
                                ...
                                placeholder={t('Select Log Event')}
                            />
                        </div>
                    );
                },
    ...
    const handleDelete = async (data) => {
        const ok = await confirm({
            ...
            description: `The customer <span class="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-bold">"${data.name}"</span> will be permanently removed. This action cannot be undone.`,
            variant: 'destructive',
            confirmText: 'Yes, Delete It',
        });

        if (ok) {
            router.delete(`/activity-logs/${data.id}`, {
                onSuccess: (response) => {
