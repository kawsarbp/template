...
import BankAccountActionButton from './BankAccountActionButton';
import { BankAccountMobileDetailSheet } from './BankAccountMobileDetailSheet';
import { BankAccountMobileViewCard } from './BankAccountMobileViewCard';
import BankAccountModal from './BankAccountModal';

const breadcrumbs = [{ label: 'Bank Accounts' }];
...
export default function BankAccounts(props) {
    ...
    const portal = usePortal('bank-account-modal');
    ...
    const tableColumnsDefinitions = useMemo(
        () => [
...

            {
                ...
                header: t('Action'),
                cell: ({ row }) =>
                    ![3, 4].includes(row.original.id) && (
                        <BankAccountActionButton
                ...
                cellMobileView: (mobileViewData) => (
                    <DetailRow label="Action">
                        {![3, 4].includes(mobileViewData.id) && (
                            <BankAccountActionButton
    ...
    const handleDelete = async (data) => {
        const ok = await confirm({
            ...
            description: `The bank account <span class="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-bold">"${data.holder_name}"</span> will be permanently removed. This action cannot be undone.`,
            variant: 'destructive',
            confirmText: 'Yes, Delete It',
        });

        if (ok) {
            router.delete(`/bank-accounts/${data.id}`, {
                onSuccess: (response) => {
            ...
            <div className="relative bg-background font-sans text-foreground">
                <div className="mb-4 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    ...
                    <div className="flex flex-wrap items-end justify-end gap-2">
                        ...
                        {permissions['create bank account'] && (
                            <Button
                                onClick={() => {
                                    setIsOpen(true);
                                    setEditData(null);
