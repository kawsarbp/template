...
const SalePaymentModal = ({
    ...
    const { data, setData, post, put, processing, errors, clearErrors, reset } =
        useForm({
            amount: editPayment?.amount ?? totalDue ?? '',
            payment_date: editPayment?.payment_date_raw ?? formatDateToYMD(new Date()),
    ...
    useEffect(() => {
        if (editPayment) {
            ...
            if (editPayment.bank_account_id) {
                setBankAccountChange({
                    value: editPayment.bank_account_id,
                    label: editPayment.bank_account_name ?? '',
                });
            }
        }
    }, [editPayment]);
    ...
    const handleSubmit = (e) => {
        ...
        if (isEditing) {
            put(`/sale-payments/${editPayment.id}`, {
                onSuccess: (response) => {
                    reset();
