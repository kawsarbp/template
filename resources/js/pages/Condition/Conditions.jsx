...
import ConditionActionButton from './ConditionActionButton';
import { ConditionMobileDetailSheet } from './ConditionMobileDetailSheet';
import { ConditionMobileViewCard } from './ConditionMobileViewCard';
...
export default function Conditions(props) {
    ...
    const portal = usePortal('condition-modal');
    ...
    const handleDelete = async (data) => {
        const ok = await confirm({
            ...
            description: `The condition <span class="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-bold">"${data.name}"</span> will be permanently removed. This action cannot be undone.`,
            variant: 'destructive',
            confirmText: 'Yes, Delete It',
        });

        if (ok) {
            router.delete(`/conditions/${data.id}`, {
                onSuccess: (response) => {
