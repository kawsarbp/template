...
import { BrandMobileDetailSheet } from './BrandMobileDetailSheet';
import { BrandMobileViewCard } from './BrandMobileViewCard';
...
export default function Brands(props) {
    ...
    const portal = usePortal('brand-modal');
    ...
    const handleDelete = async (data) => {
        const ok = await confirm({
            ...
            description: `The brand <span class="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-bold">"${data.name}"</span> will be permanently removed. This action cannot be undone.`,
            variant: 'destructive',
            confirmText: 'Yes, Delete It',
        });

        if (ok) {
            router.delete(`/brands/${data.id}`, {
                onSuccess: (response) => {
